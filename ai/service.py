...
def _build_prompt(customer_name: str, messages: list[str]) -> tuple[str, str]:
    ...
    system = (
        ...
        f"{context_block}\n"
        "Sua tarefa: analisar as mensagens do cliente e escolher UMA destas 2 ações:\n\n"
        "AÇÃO 1 — SAUDAÇÃO PADRÃO (ação padrão sempre que houver qualquer indício de problema técnico, dúvida "
        "sobre o sistema ou pedido): mencionar um bug, erro, travamento, dúvida de uso ou solicitação já é "
        "informação suficiente para agir — não espere uma descrição completa. NUNCA faça perguntas de triagem "
        "sobre o que foi reportado; isso será feito depois por um atendente humano. "
        f'Nesse caso retorne exatamente: "{fallback}"\n'
        "AÇÃO 2 — NÃO RESPONDER: use quando as mensagens forem só conversa social — cumprimento, agradecimento, "
        ...
        "Exemplos:\n"
        '- "o robô de faturamento parou de rodar hoje de manhã" → AÇÃO 1\n'
        '- "sistema dando erro 500 ao gerar boleto do cliente XPTO" → AÇÃO 1\n'
        '- "gente o robô travou de novo" → AÇÃO 1\n'
        '- "alguém pode ajudar?" (sem mais contexto) → AÇÃO 1\n'
        '- "oi pessoal" / "bom dia" / "muito obrigado!" → AÇÃO 2\n\n'
        "Regras:\n"
        "- Responda em português do Brasil.\n"
        "- Seja breve, cordial e profissional, seguindo o contexto do negócio acima quando fornecido.\n"
        "- Não invente soluções, prazos ou promessas — você está apenas acolhendo; um atendente humano vai continuar depois.\n"
